...
- ⚠️⚠️ **只做静态提取是不够的 —— 迭代了**四**轮才闭合（这是本轮最大的教训）**：
  ...
  3. **第三版**发现第二版仍有洞：`enrich.py` 的 `card_label = _PRODUCT_CARD_LABELS.get(slug) or _PRODUCT_CAT_TO_SIDEBAR_LABEL.get(...)` —— 把**数据源换成第三张 dict** 时，**调用点数量没变**（清单绿）、静态只认那两张已知映射（也绿）→ 产品卡的分类标签可以在五语下静默回退英文**而无人报警**。实测突变 M4 确认漏检后补上 **spy #2**（patch `pages.views.enrich._t`，用 seed 的 20 个真实产品驱动 `_enrich_product`）。
  4. **第四版**（独立验证者第二轮）发现**同一类洞还差一个模块**：`common.py` 的 `_t(config.hero_title, lang)` 换成 `_t(_HERO_COPY['title'], lang)` 后 —— ① 静态规则 (c) 是按 `config.<字段>` 这个 **name** 认人的，换掉后该字段**从待译集合里悄悄消失**（是「覆盖变少」，**不是变红**）；② 调用点清单数量 6→6 不变；③ 两个 spy 都不碰 `common.py`。→ 首页 hero 标题在五语下静默回退英文，而守卫 6/6 全绿。补两层：**spy #3**（patch `pages.views.common._t`，`override_settings(IS_VERCEL=True)` 走生产无状态分支真跑 `get_common_context()`）+ **`KNOWN_CONFIG_FIELDS` 字段集钉死**（`{'hero_title','hero_subtitle','products_title','products_subtitle','projects_title','projects_subtitle'}`，改为别名 `cfg = config`、`getattr(config,'x')`、或第三张 dict 全都会红）。
  - **两条可复用结论**（已写进 `MEMORY.md`）：
    - **跨模块 `from x import y` 会让 runtime spy 失效 → patch 必须打在「调用方模块」上，而不是「定义模块」上。**
