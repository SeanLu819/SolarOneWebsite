...
def _write_seed_files(seed_data, base_dir=None, write_json=True):
    ...
    ``write_json`` should be True only when the database is the source of truth
    (DB mode). In JSON mode seed_data.json IS the source of truth, so we only
    regenerate the embedded Python module and never write back to the JSON.
    """
    if base_dir is None:
        try:
            from django.conf import settings
    ...
    for key, comment in _seed_section_comments(seed_data).items():
        py_data = py_data.replace('\n  "%s":' % key,
                                  '\n\n  %s\n  "%s":' % (comment, key), 1)

    lines = [
        '"""Seed data embedded as Python module for Vercel compatibility.',
        '',
        'AUTO-GENERATED by pages/seed_sync.py — DO NOT EDIT BY HAND.',
        'Source of truth: seed_data.json (committed to the repo).',
        'Regenerate with:  python -m pages.seed_sync --json',
        '(build.sh runs this on every Vercel build, so this .py is a build',
        ' artifact and is git-ignored — it is never committed.)',
        '"""',
        '',
        'SEED_DATA = ' + py_data,
        '',
    ...
    return (os.path.join(base_dir, 'seed_data.json') if write_json else None), py_path


def _seed_section_comments(seed_data):
    """Human-readable section banners inserted into the generated .py."""
...
def sync_seed_from_json(base_dir=None):
    ...
    # JSON mode: seed_data.json is the source of truth — only regenerate the
    # embedded .py module, do NOT write back to the JSON.
    _write_seed_files(seed_data, base_dir, write_json=False)
